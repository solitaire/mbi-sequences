...
class SequencesSuite extends FlatSpec with Matchers {
  ...
  "Iterative NeedlemanWunsch" should "give same results as recursive NeedlemanWunsch" in {
    ...
    val iterative: (Int, _root_.mbi.sequences.sequences.Moves) = Sequences.iterativeNeedlemanWunschInternal(s1, s2, s3, similarityMatrix)
    val recursive: (Int, _root_.mbi.sequences.sequences.Moves) = Sequences.recursiveNeedlemanWunschInternal(s1, s2, s3, similarityMatrix)
